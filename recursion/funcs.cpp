...
void Menu() // Функция меню
{
	...
	case '1':
	{
		...
		int i = 0;
		system("cls");
		cout << "Введите натуральное число, не большее 25: ";
		for (int i = 0; i < 2; i++)
			nString[i] = _getche();
		n = atoi(nString); // Перевод строки в число
		if (n == 0 || n > 25) // Проверка верности введенного число
			do
			{
				system("cls");
				cout << "Вы ввели некорректное число либо символ, введите натуральное число, не большее 25: ";
				for (int i = 0; i < 2; i++)
					nString[i] = _getche();
				n = atoi(nString);
			} while (n == 0 || n > 25);
